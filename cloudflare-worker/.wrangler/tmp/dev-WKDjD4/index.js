...
// .wrangler/tmp/bundle-HxlPMA/checked-fetch.js
...
async function callNVIDIA(apiKey, messages) {
  ...
  console.log("[AI DIAGNOSTIC] NVIDIA Models:", JSON.stringify(modelsData));
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.error("[AI ERROR] NVIDIA API error:", res.status, errBody);
      if (res.status === 429) {
        throw new Error("RATE_LIMIT_EXCEEDED");
      } else if (res.status >= 500) {
        throw new Error(`SERVER_ERROR_${res.status}`);
...
async function callGroq(apiKey, messages) {
  ...
  const modelsData = await modelsRes.json();
  console.log("[AI DIAGNOSTIC] Groq Models:", JSON.stringify(modelsData));
  ...
  try {
    ...
    if (!res.ok) {
      const errBody = await res.text().catch(() => "");
      console.error("[AI ERROR] Groq API error:", res.status, errBody);
      if (res.status === 429) {
        throw new Error("RATE_LIMIT_EXCEEDED");
      } else if (res.status >= 500) {
        throw new Error(`SERVER_ERROR_${res.status}`);
...
// .wrangler/tmp/bundle-HxlPMA/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-HxlPMA/middleware-loader.entry.ts
